...
| TD-15 | Coolify managed PG'ye `make migrate` deploy hook | H4 (security audit) | H10 (DevOps güçlendirme) | coolify-engineer + devops-engineer | Sprint 4 migration zinciri (0001-0004) henüz Coolify managed PostgreSQL'e apply edilmedi; manuel `psql` apply yapılırsa audit trail zayıf, idempotency Coolify side'da kayıtlı değil. H10'da `make migrate` deploy hook olarak Coolify'a bağlanır (git push → migration runner → schema_migrations audit kaydı). Geçici çözüm: `docs/sprints/sprint-10-coolify-migrate.md` runbook (placeholder) ile manuel apply adımlarını sabitle. **H5 ek:** 0005 planner tuning migration'ı da aynı deploy hook kapsamında olacak; ayrıca Coolify VPS RAM'ine göre `effective_cache_size` override adımı runbook'a eklenir (`sprint-05-perf.md` → "Coolify managed PG'ye uygulama notu"). |
| TD-16 | `config/tr_holidays.yaml` yıllık manuel yenileme | H5 (sprint kickoff) | H17+ (her Aralık) | data-engineer | Sprint 5 B1 kararı: TR resmi + dini tatil seti runtime API'siz statik YAML. Maliyeti: yıllık manuel ekleme. Her Aralık ayında (veya Diyanet bir sonraki yıl bayram tarihlerini ilan ettiğinde) `holidays` listesine yeni yıl satırlarını ekle + `seed_dim_time` `SEED_END` sabitini uzat. Sprint review template'inde otomatik hatırlatıcı (Aralık sprint'lerinde checklist item). |
